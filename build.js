...
async function run() {
  ...
  await Promise.all(lessons.map(async lesson => {
    ...
    console.error(`lessons/${lesson.index + '.md'} -> dist/${lesson.index + '.html'}`);
  }));
  ...
  await Promise.all((await copy())
    .map(filePath => {
      ...
      fs.readFile(filePath)
        ...
        .then(() => console.error(`${path.basename(path.dirname(filePath))}/${fileName} -> dist/${fileName}`));
    }));
}
